...
class HaloPluginTester:
    def __init__(self):
        ...
        # 存储创建的资源ID，用于清理
        self.created_resources = {"posts": [], "moments": [], "tags": [], "categories": []}
    ...
    def test_create_post(self, tag_id=None, category_id=None):
        """测试创建文章"""
        print("\n✍️ 测试6: 创建文章")
        try:
            ...
            content_response = requests.put(
                f"{self.base_url}/apis/api.console.halo.run/v1alpha1/posts/{post_id}/content",
                headers=self.headers,
                json=content_data,
                timeout=10
            )
